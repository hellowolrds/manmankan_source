...
function getSearchRequest (keyword, page) {
    var url = "http://www.7edm.com/statics/search.aspx?key=" + keyword + "&page=" + page;
...
function getInfoRequest(cid) {
    cid = cid.replace(/_/g, "/");
    var url = "http://www.7edm.com/".concat(cid);
...
function getImagesRequest(cid, path) {
      var url = "http://www.7edm.com/" + path;
      Log.d("测试", "getImagesRequest: "+url);
      return new Request.Builder()
              .url(url).build();
}
...
function getCategoryRequest(format, page) {
   var url = "http://www.7edm.com" + format + page +".html";
